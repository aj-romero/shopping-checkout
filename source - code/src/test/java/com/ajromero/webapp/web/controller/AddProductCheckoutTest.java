...
@DisplayName("Add Product to Checkout Controller test")
class AddProductCheckoutTest {
    @Mock
    ...
    @Test
    @DisplayName("Add product to Checkout success")
    void whenAddNewProductToCheckout_thenAssert() {
        ...
        when(checkoutService.addProduct(anyLong(),any())).thenReturn(dtoBasicCk);

        ResponseEntity<CheckoutBasicDto> result = checkoutController.addProductCheckout(1l,dtoCkProduct);
        ResponseEntity<CheckoutBasicDto> expected = ResponseEntity.
                status(HttpStatus.CREATED).body(dtoBasicCk);

        verify(checkoutService).addProduct(anyLong(),any());
