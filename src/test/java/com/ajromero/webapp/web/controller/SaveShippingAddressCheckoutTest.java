...
@DisplayName("Save Shipping Address - Checkout Controller test")
class SaveShippingAddressCheckoutTest {
    @Mock
    ...
    @Test
    @DisplayName("Save shipping address to Checkout success")
    void whenSaveShippingAddressToCheckout_thenAssert() {
        CheckoutWithShippingDto dtoShippingCk = new CheckoutWithShippingDto();
        ShippingDto dtoShipping = new ShippingDto();

        when(checkoutService.saveShippingAddress(anyLong(),any())).thenReturn(dtoShippingCk);

        ResponseEntity<CheckoutWithShippingDto> result = checkoutController.saveShippingAddress(1l,dtoShipping);
        ResponseEntity<CheckoutWithShippingDto> expected = ResponseEntity.
                status(HttpStatus.CREATED).body(dtoShippingCk);

        verify(checkoutService).saveShippingAddress(anyLong(),any());
